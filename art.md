...
| Grid | faint blue `#4a7fa8`, barely visible |
...
- **Fades and darkens to pure black at all four edges** — a heavy vignette. This
  is what lets tiles sit seamlessly on the dark base plate, so it matters.
...
watermarks, angled or isometric perspective, daylight, white or light
backgrounds, cartoon outlines, clean modern surfaces.
...
```
Top-down overhead fantasy battle-map tile of [SUBJECT].
...
used and slightly ruined. A faint blue grid is barely visible over the floor.
Painterly brushwork, rich texture, high detail, no linework or cartoon outlines.

Composition: the location fills the centre of a square frame with generous empty
margin around it, and the scene fades and darkens to pure black at all four edges
with a heavy vignette, so it can sit seamlessly on a black background.

Do not include: text, labels, numbers, letters, people, creatures, UI elements,
borders, frames, watermarks, angled or isometric perspective, daylight, white or
light backgrounds.
```
...
**Save as:** `frontend/public/map/base.webp` · **Size:** landscape, 1536×1024
...
distortion. Almost entirely deep shadow and near-black rock, with a faint blue
grid barely visible across it and a few very dim pools of distant amber light in
the corners. Damp bedrock, scattered rubble, cracks and old scorch marks.
Painterly brushwork, subtle texture, very low contrast, nothing bright.

This is an empty background plate: no rooms, no structures, no doors, no
corridors, no focal point, no people, no creatures, no text, no borders. It must
read as empty dark ground that other artwork will be placed on top of.
```
...
22 tiles. Save each as `frontend/public/map/zones/<filename>`.
...
| --- | --- | --- | --- |
| 1 | Intro | `intro.webp` | a small orientation chamber with cracked motivational banners and a single lit doorway |
| 2 | Networking | `networking.webp` | stone canals of glowing teal data flowing under narrow footbridges |
| 3 | Governance, Risk & Compliance | `governance-risk-compliance.webp` | a vast archive hall of chained ledgers and toppled filing stacks |
| 4 | Hacker Game Show | `hacker-game-show.webp` | a lit arena stage ringed by buzzer podiums and dead spotlights |
| 5 | CTI | `cti.webp` | a trophy hall of broken siege weapons mounted on stone walls |
| 6 | Incident Response | `incident-response.webp` | a burned-out server hall of collapsed racks, rubble and glowing embers |
| 7 | AI/LLM Security | `ai-llm-security.webp` | a shrine chamber built around one vast glowing eye set into the floor |
| 8 | Prompt Injection | `prompt-injection.webp` | a whispering gallery lined with carved stone mouths |
| 9 | Forensics | `forensics.webp` | a frozen morgue of open specimen drawers and hanging tape reels |
| 10 | Threat Detection | `threat-detection.webp` | a ring of watchtowers casting sweeping amber lantern beams |
| 11 | Cloud Security | `cloud-security.webp` | a molten foundry of pipes, vents and glowing forges |
| 12 | OSINT | `osint.webp` | an open-air records court strewn with unrolled maps and pinned notes |
| 13 | Red teaming | `red-teaming.webp` | a war room around a great siege table covered in battle plans |
| 14 | Hardware Hacking | `hardware-hacking.webp` | a workbench pit of solder irons, cabling and exposed circuit boards |
| 15 | Social Engineering | `social-engineering.webp` | a masquerade bazaar of false storefronts and hanging masks |
| 16 | Identity & Access | `identity-access.webp` | a labyrinth of numbered doors with brass badge readers |
| 17 | Web Attacks | `web-attacks.webp` | a caustic glowing green slime marsh creeping over cracked tiling |
| 18 | Codes and Ciphers | `codes-and-ciphers.webp` | a frozen vault of enormous rotating brass cipher rings |
| 19 | Crypto | `crypto.webp` | a sealed sanctum of tall glowing glyph-carved pillars |
| 20 | Mobile Security | `mobile-security.webp` | a shrine of hand-sized glowing slabs on stone plinths |
| 21 | Reverse Engineering | `reverse-engineering.webp` | a dissection hall of opened machines with their guts laid out |
| 22 | Malware Analysis | `malware-analysis.webp` | a sealed quarantine cell marked with glowing warning sigils |
...
| Hard edges or a visible frame | Re-emphasise *"fades to pure black at all four edges, heavy vignette, no border"* |
